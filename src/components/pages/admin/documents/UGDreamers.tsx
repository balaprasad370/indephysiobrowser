...
const UGDreamers = ({ studentDetails, onAccept, onReject, student_id }) => {
  ...
  const getDocuments = async () => {
    ...
    response.data.map((doc) => {
      if (doc.document_type === "firstyearmarksheet") {
        _documents.firstyearmarksheet = { ...doc };
      }
    });
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[{ name: "firstyearmarksheet", label: "First Year Marksheet" }].map(
          (document, index) => (
            <div key={index}>
