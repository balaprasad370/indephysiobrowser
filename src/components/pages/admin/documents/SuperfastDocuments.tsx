...
const SuperfastDocuments = ({ studentDetails, student_id, onAccept, onReject }) => {
  ...
  const getDocuments = async () => {
    ...
    response.data.map((doc) => {
      if (doc.document_type === "degreecertificate") {
        _documents.degreecertificate = { ...doc };
      }
      if (doc.document_type === "coursetranscript") {
        _documents.coursetranscript = { ...doc };
      }
      if (doc.document_type === "iaporcouncil") {
        _documents.iaporcouncil = { ...doc };
      }
      if (doc.document_type === "birthcertificate") {
        _documents.birthcertificate = { ...doc };
      }
    });
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          { name: "coursetranscript", label: "Course Transcript" },
          { name: "iaporcouncil", label: "IAP or Council Certificate" },
                ...
                documents[document.name]?.document_path.includes(".pdf") ? (
                  ...
                  <div className="flex items-center justify-center pb-4">
                    <img
                      className="h-[120px] w-full object-contain"
                      src={
                        filesServerUrl + documents[document.name]?.document_path
                      }
                      alt={document.name}
                    />
                  </div>
...
export default SuperfastDocuments;
